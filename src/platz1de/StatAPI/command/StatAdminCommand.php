...
class StatAdminCommand extends CommandBase
{
	...
	public function execute(CommandSender $sender, string $commandLabel, array $args): void
	{
		...
		switch (strtolower($args[0] ?? "help")) {
			case "modules":
				$sender->sendMessage(StatAPI::getPrefix() . str_replace(["{modules}"], [implode(", ", $this->getOwningPlugin()->getModules())], StatAPI::getInstance()->getConfig()->get("statadmin-modules", "§eList of all modules:§r\n{modules}")));
				return;
			case "stats":
				if (count($args) > 1 && ($module = $this->getOwningPlugin()->getModule($args[1], false)) instanceof Module) {
			...
			case "deletemodule":
				if (count($args) > 1 && ($module = $this->getOwningPlugin()->getModule($args[1], false)) instanceof Module) {
					...
					$this->getOwningPlugin()->getDatabase()->executeChange(Query::UNREGISTER_MODULE, ["module" => $module->getName()], function (int $affectedRows) use ($sender, $module) {
						...
						$this->getOwningPlugin()->unregisterModule($module);
					});
				} else {
					$sender->sendMessage(StatAPI::getPrefix() . str_replace(["{module}"], [$args[1] ?? ""], StatAPI::getInstance()->getConfig()->get("statadmin-module-unknown", "The module '{module}' doesn't exists!")));
				}
				return;
			case "deletestat":
				if (count($args) > 2 && ($stat = $this->getOwningPlugin()->getStat($args[2], $this->getOwningPlugin()->getModule($args[1]))) instanceof Stat) {
					$this->getOwningPlugin()->getDatabase()->executeChange(Query::UNREGISTER_STAT, ["stat" => $stat->getName(), "module" => $stat->getModule()->getName()], function (int $affectedRows) use ($sender, $stat) {
			...
			case "showstat":
				...
				return;
			case "setmodulename":
				if (count($args) > 2 && ($module = $this->getOwningPlugin()->getModule($args[1], false)) instanceof Module) {
				...
				return;
			case "setstatname":
				if (count($args) > 3 && ($stat = $this->getOwningPlugin()->getStat($args[2], $this->getOwningPlugin()->getModule($args[1]))) instanceof Stat) {
			...
			case "resetscores":
				if (count($args) > 1) {
					$this->getOwningPlugin()->getDatabase()->executeChange(Query::REMOVE_PLAYER_DATA, ["player" => $args[1]], function (int $affectedRows) use ($sender, $args) {
				...
				return;
			case "setscore":
				if (count($args) > 4 && ($stat = $this->getOwningPlugin()->getStat($args[2], $this->getOwningPlugin()->getModule($args[1]))) instanceof Stat) {
					try {
			...
			case "rankstat":
				if (count($args) > 3 && ($stat = $this->getOwningPlugin()->getStat($args[2], $this->getOwningPlugin()->getModule($args[1]))) instanceof Stat) {
					$currentPosition = max(0, 1 + array_search(strtolower($stat->getName()), array_keys($stat->getModule()->getStats()), true));
					$afterPosition = max(0, $currentPosition - $args[3]);
					if ($args[3] < 0) {
						$pos = $afterPosition;
						$this->getOwningPlugin()->getDatabase()->executeChange(Query::SET_STAT_POSITION, ["stat" => $stat->getName(), "module" => $stat->getModule()->getName(), "position" => $pos]);
						foreach (array_slice($stat->getModule()->getStats(), $afterPosition) as $s) {
							$this->getOwningPlugin()->getDatabase()->executeChange(Query::SET_STAT_POSITION, ["stat" => $s->getName(), "module" => $s->getModule()->getName(), "position" => ++$pos]);
						}
						$pos = $currentPosition;
						...
						$this->getOwningPlugin()->getDatabase()->executeChange(Query::SET_STAT_POSITION, ["stat" => $stat->getName(), "module" => $stat->getModule()->getName(), "position" => $afterPosition]);
					}
