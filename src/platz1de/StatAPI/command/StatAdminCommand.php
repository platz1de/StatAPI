	...
	public function execute(CommandSender $sender, string $commandLabel, array $args): void
	{
		...
		switch (strtolower($args[0] ?? "help")) {
			case "modules":
				...
				return;
			case "stats":
				if (count($args) > 1 && ($module = $this->getPlugin()->getModule($args[1], false)) instanceof Module) {
			...
			case "deletemodule":
				...
				return;
			case "deletestat":
				if (count($args) > 2 && ($stat = $this->getPlugin()->getStat($args[2], $this->getPlugin()->getModule($args[1]))) instanceof Stat) {
			...
			case "showstat":
				...
				return;
			case "setmodulename":
				if (count($args) > 2 && ($module = $this->getPlugin()->getModule($args[1], false)) instanceof Module) {
				...
				return;
			case "setstatname":
				if (count($args) > 3 && ($stat = $this->getPlugin()->getStat($args[2], $this->getPlugin()->getModule($args[1]))) instanceof Stat) {
			...
			case "resetscores":
				...
				return;
			case "setscore":
				if (count($args) > 4 && ($stat = $this->getPlugin()->getStat($args[2], $this->getPlugin()->getModule($args[1]))) instanceof Stat) {
					try {
